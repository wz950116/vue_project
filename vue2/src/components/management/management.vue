<template>
    <div>
        <div class="manage-search">
            <el-select v-model="type" class="mang-select" @change="search(1)">
                <el-option v-for="item in typeOptions" :label="item.description" :value="item.roleId" ></el-option>
            </el-select>
        ...
        <div class="manage-list">
            ...
            <div class="pagination">
                <el-pagination v-model="currentPage" v-show="this.tableData.length>0" @current-change="search"
        ...
        <!--dialog-->
        <el-dialog title="添加权限" v-model="dialogFormVisible" size="tiny">
          <el-form :model="manageForm" label-width="60px">
            ...
            <el-form-item label="角色">
              <el-select v-model="manageForm.type" class="mang-select">
                <el-option v-for="(item, key) in typeOptions" v-if="key > 0" :label="item.description" :value="item.roleName" ></el-option>
              </el-select>
            </el-form-item>
          </el-form>
        ...
        <!--edit role dialog -->
        <el-dialog @close="editDialogClose" title="编辑角色" v-model="dialogRoleEditVisible" size="tiny">
            ...
            <el-select class="addRoles" v-model="addRoles" multiple placeholder="请选择">
                <el-option filterable multiple v-for="item in allPermissions" :label="item.perm_des" :value="item.perm_id"></el-option>
            </el-select>
            <el-button type="primary" @click="addRoleItem" style="float:right;margin-bottom:20px;">添 加</el-button>
            <el-table :data="editRoleData" border style="width:100%;" align="center" max-height="450">
                <el-table-column inline-template label="角色名">
                    <div>
                        <span v-if="currentEdit !== $index">{{row.description}}</span>
                ...
                <el-table-column inline-template label="权限" width="" align="center">
                    <el-select v-show="currentEdit === $index" class="permissionSelect" v-model="editRoleData[$index].options" multiple placeholder="请选择">
                        <el-option filterable multiple v-for="item in allPermissions" :label="item.perm_des" :value="item.perm_id">
                        </el-option>
                    </el-select>
                </el-table-column>
                <el-table-column inline-template label="操作" align="center">
                    <div>
                        <span v-if="currentEdit !== $index">
                            <el-button type="text" @click="editRoleItem(row, $index)">编辑</el-button>
                            <el-button type="text" @click="delRoleItem(row)">删除</el-button>
                        </span>
                        <span v-else>
                            <el-button type="text" @click="saveRoleItem(row, $index)">确认</el-button>
                        </span>
                    </div>
                </el-table-column>
            </el-table>
...
<style>
/* fix bug http://jira.ops.focus.cn/browse/ADS-952 */
.el-select-dropdown {
...
<script>
    import management from "../../script/data/model/management";
    export default{
        name: "management",
        data () {
            return {
        ...
        methods: {
            search: function (val) {
                ...
                window.localStorage.setItem("ad_management_searchContent", roleId);
        ...
        beforeMount: function () {
            ...
            management.on("getRoleList", function (data) {
                ...
                if (window.localStorage.getItem("ad_management_searchContent") != "") {
                    that.type = parseInt(window.localStorage.getItem("ad_management_searchContent")) || 0;
                }

                that.isRequire = true;
            });
            // 获取权限人列表
            management.on("getManagementList", function (res) {
                if (parseInt(res.code) === 200) {
                    const data = res.data;
                    that.tableData = data.authorityPersonList;
                    that.total = parseInt(data.totalNum);
                }
            });
            // 填加权限人回调
            management.on("addManagementById", function (data) {
                if (parseInt(data.code) === 200) {
            ...
            // 获取所有角色及其权限
            management.on("getAllRolesAndPermissions", function (data) {
                if (parseInt(data.code) === 200) {
                    const tmp = [];
                    data = data.data;
                    for (let i = 0, length = data.length; i < length; i++) {
            ...
            // 获取权限列表
            management.on("listAllPermission", function (data) {
                if (parseInt(data.code) === 200) {
