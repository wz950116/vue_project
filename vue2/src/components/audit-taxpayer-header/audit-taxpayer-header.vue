...
<script>
    ...
    export default{
        ...
        beforeMount: function () {
            ...
            const insertEnd = this._dateTimeFormatter(new Date(), "YYYY-MM-DD") + " 23:59:59";
            ...
            if (window.localStorage.getItem("audit_taxpayer_searchContent")) {
                const storageObj = JSON.parse(window.localStorage.getItem("audit_taxpayer_searchContent"));
        ...
        methods: {
            ...
            getParams: function (type) {
                ...
                if (!this.submitTimeDisable && this.submitDateRange != "" && this.submitDateRangeForView.length > 0 && this.submitDateRangeForView[0]) {
                    _.extend(params, {
                        insertStart: this.submitDateRange.split(" - ")[0],
                        insertEnd: this.submitDateRange.split(" - ")[1]
                    });
                }
                if (!this.auditTimeDisable && this.auditDateRange != "" && this.auditDateRangeForView.length > 0 && this.auditDateRangeForView[0]) {
                    _.extend(params, {
                ...
                window.localStorage.setItem("audit_taxpayer_searchContent", JSON.stringify({
